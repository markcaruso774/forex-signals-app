...
import json
import requests
# The "import feedparser" line that was here is GONE. This is the fix.
...
# === 7. MAIN APP PAGE ===
    ...
    with st.expander("👋 Welcome to PipWizard! Click here to learn about the app."):
        st.markdown(
            ...
            * ✅ **Economic News Calendar** (Demo data, real-time feed with Premium API key)
            ...
            * ✅ **🚀 Strategy Scanner**
            * ✅ **Live Signal Alerts**
            *(A paid Twelve Data API plan is required for the live Calendar & high-speed Scanner)*
            """
        )
    ...
    # --- RECTIFIED: display_news_calendar (Reverted to "Smart Mock" to avoid 404 API error) ---
    def display_news_calendar():
        ...
        days_to_monday = (7 - today_obj.weekday()) % 7 
        if days_to_monday == 0: days_to_monday = 7 # If today is Monday, get next Monday
            
        next_monday = today_obj + pd.Timedelta(days=days_to_monday)
        next_tuesday = next_monday + pd.Timedelta(days=1)
        next_wednesday = next_monday + pd.Timedelta(days=2)
        next_thursday = next_monday + pd.Timedelta(days=3)
        next_friday = next_monday + pd.Timedelta(days=4)

        # "Smart Mock" events. These are placeholders.
        events = [
            {"date": next_monday, "time": "14:00", "event": "German Flash PMI", "impact": "Medium"},
            {"date": next_tuesday, "time": "09:30", "event": "UK Claimant Count Change", "impact": "Medium"},
            {"date": next_tuesday, "time": "15:00", "event": "US CB Consumer Confidence", "impact": "High"},
            {"date": next_wednesday, "time": "01:30", "event": "AUD CPI (Q/Q)", "impact": "High"},
            {"date": next_wednesday, "time": "15:00", "event": "CAD Interest Rate Decision", "impact": "High"},
            {"date": next_thursday, "time": "12:30", "event": "ECB Interest Rate Decision", "impact": "High"},
            {"date": next_friday, "time": "13:30", "event": "US Nonfarm Payrolls", "impact": "High"},
            {"date": next_friday, "time": "13:30", "event": "US Unemployment Rate", "impact": "High"},
        ]
        ...
        st.caption("This is a demo calendar. A paid Twelve Data API plan will enable a real-time feed.")
    ...
    st.markdown("---")
    display_news_calendar() # <-- This now calls the reverted, safe mock function
    st.markdown("---")
